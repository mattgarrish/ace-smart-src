...
if (!function_exists('securePage')) {
    function securePage($uri)
    {
        ...
        if (empty($pageDetails)) {
            ...
            // MG - escape ampersands in redirect URL otherwise all parameters but first are stripped on redirect
            $esc_redir = str_replace('%26','%2526',$dest);
            
            Redirect::to($us_url_root.'users/login.php?dest='.$page.'&redirect='.$esc_redir);
