...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="http://{$_SERVER["HTTP_HOST"]}/css/report.css"/>
HTML;

	if ($_POST['modules']) {
		foreach (explode(',', $_POST['modules']) as $module) {
			$html .= '<link rel="stylesheet" type="text/css" href="http://' . $_SERVER["HTTP_HOST"] . '/extensions/' . $module . '/css/report.css"/>'; # . '?' . time() . '"/>\n';
		}
	}
	
	$html .= <<<HTML
		<link rel="stylesheet" type="text/css" href="http://{$_SERVER["HTTP_HOST"]}/css/tabs.css"/>
	...
	<body typeof="Book" vocab="https://schema.org">
		...
		<footer>
			<p id="timestamp">Generated <span id="date-created">{$_POST['timestamp']}</span> using <a href="http://smart.daisy.org">Ace SMART</a></p>
